(function (o, s) {
  typeof exports == 'object' && typeof module < 'u'
    ? s(exports, require('earcut'), require('mapbox-gl'))
    : typeof define == 'function' && define.amd
      ? define(['exports', 'earcut', 'mapbox-gl'], s)
      : ((o = typeof globalThis < 'u' ? globalThis : o || self),
        s(
          (o.MapboxGlInterpolateHeatmap = o.MapboxGlInterpolateHeatmap || {}),
          o.earcut,
          o.mapboxgl,
        ));
})(this, function (o, s, m) {
  ...
  `),
        (this.opacity = t.opacity ?? 0.5),
        (this.minValue = t.minValue ?? 1 / 0),
        (this.maxValue = t.maxValue ?? -1 / 0),
        (this.p = t.p ?? 3),
        (this.framebufferFactor = t.framebufferFactor ?? 0.3),
    ...
    onAdd(t, e) {
      ...
      const i = `
        ...
        A = `
          ...
          `,
        x = p(e, w);
      if (!x) throw new Error('error: computation vertex shader not created');
      const _ = E(e, A);
      if (!_) throw new Error('error: computation fragment shader not created');
      if (((this.computationProgram = b(e, x, _)), !this.computationProgram))
        throw new Error('error: computation fragment shader not created');
      if (
        ((this.aPositionComputation = e.getAttribLocation(
        ...
        (this.uUi = e.getUniformLocation(this.computationProgram, 'ui')),
        (this.uXi = e.getUniformLocation(this.computationProgram, 'xi')),
        (this.uP = e.getUniformLocation(this.computationProgram, 'p')),
        (this.uFramebufferSize = e.getUniformLocation(
          this.computationProgram,
          'u_FramebufferSize',
        )),
      ...
      const v = p(e, i);
      if (!v) throw new Error('error: drawing vertex shader not created');
      const F = E(e, u);
      if (!F) throw new Error('error: drawing fragment shader not created');
      if (((this.drawProgram = b(e, v, F)), !this.drawProgram))
        throw new Error('error: drawing program not created');
      if (
        ((this.aPositionDraw = e.getAttribLocation(
          this.drawProgram,
          'a_Position',
        )),
        (this.uMatrixDraw = e.getUniformLocation(this.drawProgram, 'u_Matrix')),
        (this.uComputationTexture = e.getUniformLocation(
          this.drawProgram,
        ...
        (this.uOpacity = e.getUniformLocation(this.drawProgram, 'u_Opacity')),
      ...
      const h = [];
      this.aoi?.length === 0
        ? h.push(-1, -1, -1, 1, 1, 1, 1, -1)
        : this.aoi?.forEach((a) => {
            const c = m.MercatorCoordinate.fromLngLat(a);
            h.push(c.x, c.y);
          }),
        (this.drawingVerticesBuffer = e.createBuffer()),
        e.bindBuffer(e.ARRAY_BUFFER, this.drawingVerticesBuffer),
        e.bufferData(e.ARRAY_BUFFER, new Float32Array(h), e.STATIC_DRAW);
      const S = this.textureCoverSameAreaAsROI
        ? h
        : [1, 1, -1, 1, 1, -1, -1, -1];
      (this.computationVerticesBuffer = e.createBuffer()),
        e.bindBuffer(e.ARRAY_BUFFER, this.computationVerticesBuffer),
      ...
      (this.indicesNumber = R.length),
        e.bindBuffer(e.ELEMENT_ARRAY_BUFFER, this.indicesBuffer),
        e.bufferData(e.ELEMENT_ARRAY_BUFFER, new Uint8Array(R), e.STATIC_DRAW),
        ...
        (this.computationTexture = e.createTexture()),
        e.bindTexture(e.TEXTURE_2D, this.computationTexture),
        ...
        (this.computationFramebuffer = e.createFramebuffer()),
        e.bindFramebuffer(e.FRAMEBUFFER, this.computationFramebuffer),
      ...
      this.data.forEach((a) => {
        const c = m.MercatorCoordinate.fromLngLat(a);
        this.points.push([c.x, c.y, a.val]),
          a.val < n && (n = a.val),
          a.val > f && (f = a.val);
      }),
        (n = n < this.minValue ? n : this.minValue),
        (f = f > this.maxValue ? f : this.maxValue),
    ...
    onRemove(t, e) {
      ...
      t.off('resize', this.resizeFramebuffer),
        e.deleteTexture(this.computationTexture),
        e.deleteBuffer(this.drawingVerticesBuffer),
        e.deleteBuffer(this.computationVerticesBuffer),
        e.deleteBuffer(this.indicesBuffer),
        e.deleteFramebuffer(this.computationFramebuffer);
    }
    prerender(t, e) {
      if (
        !this.framebufferWidth ||
        !this.framebufferHeight ||
        this.aPositionComputation === void 0 ||
        !this.indicesNumber ||
        !this.canvas ||
        !this.canvas.width ||
        !this.canvas.height
      ...
      t.disable(t.DEPTH_TEST),
        ...
        t.useProgram(this.computationProgram),
        t.uniformMatrix4fv(this.uMatrixComputation, !1, e),
        t.uniform1f(this.uP, this.p),
        t.uniform2f(
          this.uFramebufferSize,
          this.framebufferWidth,
          this.framebufferHeight,
        ),
        t.bindFramebuffer(t.FRAMEBUFFER, this.computationFramebuffer),
        t.viewport(0, 0, this.framebufferWidth, this.framebufferHeight),
        t.clear(t.COLOR_BUFFER_BIT | t.DEPTH_BUFFER_BIT),
        t.bindBuffer(t.ELEMENT_ARRAY_BUFFER, this.indicesBuffer);
      for (let i = 0; i < this.points.length; i += 1) {
        const u = this.points.at(i);
        if (!u) throw new Error(`error: point not found at index: ${i}`);
        t.uniform1f(this.uUi, u[2]),
          t.uniform2f(this.uXi, u[0], u[1]),
          t.bindBuffer(t.ARRAY_BUFFER, this.computationVerticesBuffer),
          t.enableVertexAttribArray(this.aPositionComputation),
          ...
          this.textureCoverSameAreaAsROI
            ? t.drawElements(
                t.TRIANGLES,
                this.indicesNumber,
                t.UNSIGNED_BYTE,
    ...
    render(t, e) {
      ...
      t.useProgram(this.drawProgram),
        t.bindBuffer(t.ARRAY_BUFFER, this.drawingVerticesBuffer),
        t.enableVertexAttribArray(this.aPositionDraw),
        t.vertexAttribPointer(this.aPositionDraw, 2, t.FLOAT, !1, 0, 0),
        t.uniformMatrix4fv(this.uMatrixDraw, !1, e),
        t.activeTexture(t.TEXTURE0),
        t.bindTexture(t.TEXTURE_2D, this.computationTexture),
        t.uniform1i(this.uComputationTexture, 0),
        t.uniform2f(
          this.uScreenSizeDraw,
          this.canvas.width,
          this.canvas.height,
        ),
        t.uniform1f(this.uOpacity, this.opacity),
        t.blendFunc(t.SRC_ALPHA, t.ONE),
        t.bindBuffer(t.ELEMENT_ARRAY_BUFFER, this.indicesBuffer),
        t.drawElements(t.TRIANGLES, this.indicesNumber, t.UNSIGNED_BYTE, 0);
    }
  }
  function p(r, t) {
  ...
  (o.MapboxInterpolateHeatmapLayer = d),
    (o.default = d),
